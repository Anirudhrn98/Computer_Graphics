...
function setUpTextures(){
  ...
  cherryTexture = gl.createTexture();
  gl.bindTexture(gl.TEXTURE_2D, cherryTexture);
  
  // load the actual image
  var cherryImage = document.getElementById ('cherry-texture')
  cherryImage.crossOrigin = "";
  ...
  // load the texture data
  gl.texImage2D(gl.TEXTURE_2D, 0, gl.RGBA, cherryImage.width, cherryImage.height, 0, gl.RGBA, gl.UNSIGNED_BYTE, cherryImage);
...
function drawCurrentShape () {
    ...
    gl.bindTexture (gl.TEXTURE_2D, cherryTexture);
    gl.uniform1i (program.uCherryTexture, 1);
    }
    else{
...
function initProgram (vertexid, fragmentid) {
  ...
  program.uCherryTexture=gl.getUniformLocation(program,'cherryTexture');
